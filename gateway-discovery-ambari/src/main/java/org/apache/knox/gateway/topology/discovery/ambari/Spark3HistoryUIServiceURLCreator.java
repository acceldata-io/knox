...
/**
 * A ServiceURLCreator implementation for SPARK3HISTORYUI. Spark3 is registered as a distinct
 * Ambari service/component (SPARK3_JOBHISTORYSERVER) from Spark1/2, and Knox likewise exposes it
 * under a distinct role with its own context (/spark3history/), so it needs its own creator
 * rather than an additional fallback on SparkHistoryUIServiceURLCreator's SPARKHISTORYUI role.
 */
public class Spark3HistoryUIServiceURLCreator extends SparkCommonServiceURLCreator {

  private static final String RESOURCE_ROLE = "SPARK3HISTORYUI";
  ...
  public void init(AmbariCluster cluster) {
    super.init(cluster);
    primaryComponentName   = "SPARK3_JOBHISTORYSERVER";
    secondaryComponentName = "SPARK3_JOBHISTORYSERVER";
